// Package session derives the container and tmux-session names corral uses
...
// corral_<basename>_<sha8>. The basename keeps the name human-readable; the
...
// required for running multiple corrals on the same repo at once. Mirrors
...
func ContainerNameForWorkspace(workspace string) string {
	...
	return fmt.Sprintf("corral_%s_%x", base, h[:4])
}
...
// or session:window), so a container name like "corral_my.app" (a project in
// a directory named "my.app") would make tmux look for window "app" in session
// "corral_my" — new-session/has-session/kill-session/capture-pane all
...
// enough for our names (all begin with the fixed "corral_" prefix).
...
// (corral_<workspace-basename>), matching the name used by start/dev. Falls back
// to the legacy bare "corral" if no project config is readable.
...
func DetachedSessionName() (session string, container string, err error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("no project configured — run corral init first")
	}
