...
DOCKERCFG
    ...
    #
    # The CA cert (~/.mitmproxy/mitmproxy-ca-cert.pem) is generated by the HOST
    # mitmproxy on first run and bind-mounted in. On a slow/degraded CI boot the
    # host may not have finished writing it when we reach this line — so poll
    # briefly rather than testing once. Testing once is the flaky-test root cause:
    # a missed cert here means the cert-injector below never starts and
    # cert-injector.log never exists, which no test-side retry can recover.
    # Only wait when the proxy is actually running (firewall on). With
    # --disable-firewall there is no mitmproxy and no CA will ever appear, so
    # skip the wait to avoid a pointless 30s stall.
    ...
    if [ -f "$CA_SRC" ]; then
        sudo cp "$CA_SRC" /etc/proxy-ca.crt
        sudo chmod 644 /etc/proxy-ca.crt
        echo "✅ Proxy CA cert available at /etc/proxy-ca.crt"
    else
        echo "⚠️  Proxy CA cert not found at $CA_SRC after 30s — cert-injector will not start"
    fi
